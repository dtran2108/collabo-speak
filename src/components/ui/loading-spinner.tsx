...
function CollaboSpeakLogo({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  useEffect(() => {
    if (!svgRef.current || !blueCircleRef.current || !yellowCircleRef.current || !pinkCircleRef.current) return

    // Create a timeline for smooth, coordinated animations
    const tl = gsap.timeline({ repeat: -1 })

    // Animate circles with a gentle floating effect
