...
export default function Page() {
  ...
  const loadSession = useCallback(async () => {
    try {
      ...
      if (!sessionData) {
        ...
        setErrorObject(notFoundError)
        setError('Session not found')
        errorHandler.logError(notFoundError, { sessionId, action: 'getSessionById' })
        return
      }
      ...
      errorHandler.logError(errorObj, { sessionId, action: 'loadSession' })
    } finally {
      setLoading(false)
    }
  }, [sessionId])
  ...
  if (error || !session) {
    // Use mobile-specific error UI if available
    if (errorObject && mobileErrorHandler.shouldShowMobileSpecificUI()) {
      return (
        <MobileErrorUI
          error={errorObject}
          onRetry={loadSession}
            ...
            <Card className="w-full h-full gap-0">
              ...
              <CardContent className="py-4">
                <ErrorBoundary
                  ...
                  fallback={
                    <MobileErrorUI
                      error={new Error('Conversation component failed to load')}
